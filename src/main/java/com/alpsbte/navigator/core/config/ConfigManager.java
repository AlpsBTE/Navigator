...
public class ConfigManager {
    ...
    public ConfigManager() throws ConfigNotImplementedException {
        this.configFile = Paths.get(NavigatorPlugin.getPlugin().getDataFolder().getAbsolutePath(), "config.yml").toFile();
        ...
        if (!configFile.exists()) {
            if (this.createConfig()) {
                ...
                NavigatorPlugin.getPlugin().getServer().getPluginManager().disablePlugin(NavigatorPlugin.getPlugin());
    ...
    public boolean createConfig() {
        try {
            if (configFile.createNewFile()) {
                try (InputStream defConfigStream = NavigatorPlugin.getPlugin().getResource("defaultConfig.yml")) {
    ...
    private boolean updateConfig() {
        ...
        try {
            ...
            Bukkit.getConsoleSender().sendMessage(ChatColor.GOLD + "Successfully updated AlpsBTE-Navigator config to version " + Config.VERSION);
            return true;
        } catch (IOException ex) {
